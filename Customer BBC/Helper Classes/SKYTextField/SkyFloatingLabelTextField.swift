...
@IBDesignable
 class SkyFloatingLabelTextField: UITextField { // swiftlint:disable:this type_body_length
    ...
    fileprivate func updatePlaceholder() {
        guard let placeholder = placeholder else { //guard let placeholder = placeholder, let font = placeholderFont ?? font else {
            return
        }
     //   let color = isEnabled ? placeholderColor : disabledColor
      
        self.attributedPlaceholder = getAttrbText(char: "*", text: placeholder)//NSAttributedString(string: placeholder,attributes:
        
    }

    /// A UIFont value that determines the text font of the title label
    @objc dynamic open var titleFont: UIFont = UIFont(name: "Montserrat-Regular", size: 13)! {
        didSet {
            updateTitleLabel()
        }
        }
...
//           get {
//               guard leftView != nil else{ return 0}
...
//       }
    ...
    func updateViewRight() {
        if let image = rightImage {
            ...
            containerView.insertSubview(imageView, belowSubview: self)
            // Note: In order for your image to use the tint color, you have to select the image in the Assets.xcassets and change the "Render As" property to "Template Image".
            containerView.frame.origin.x = self.bounds.origin.x
            containerView.frame.origin.y = self.bounds.origin.y
            containerView.frame.size = CGSize.init(width: 25, height: self.bounds.size.height)
            imageView.center.y = containerView.center.y + 8
            imageView.center.x = containerView.center.x
            imageView.frame.size = CGSize.init(width: 15, height: 15)
    ...
    */
    ...
    fileprivate func updateTitleLabel(_ animated: Bool = false) {
        ...
        titleLabel.attributedText = getAttrbText(char: "*", text: titleText!)
       // titleLabel.font = titleFont

        updateTitleVisibility(animated)
    }
    fileprivate func getAttrbText(char:String,text:String) -> NSMutableAttributedString{
        
        let range = (text as NSString).range(of: String(text))
        let range1 = (text as NSString).range(of: String(char))
        
        let attribute = NSMutableAttributedString.init(string: text)
        attribute.addAttribute(NSAttributedString.Key.foregroundColor, value:titleColor, range: range)
       // hexStringToUIColor(hex: Color.fontDarkGray.rawValue)
